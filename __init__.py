    ...
    try:
        ...
        def _on_backend_ready(actual_port) -> None:
            ...
            if proxy_target is not None:
                ...
                log.info(
                    ...
                    "; stateless" if settings.mcp_stateless else "; session mode (async push on)",
                )
            else:
                log.info(
        ...
        loop.create_task(
            mcp_server.serve_embedded(
                ...
                fallback_auto=True,
                # MCP_STATELESS 默认 true：无会话模式，agent 不怕 ComfyUI 重启；
                # 只有显式设成 false 才回到有状态模式（换来异步任务的完成推送）。
...
from .nodes import NODE_CLASS_MAPPINGS as _NODE_MAPPINGS  # noqa: E402
from .nodes import NODE_DISPLAY_NAME_MAPPINGS as _NODE_NAME_MAPPINGS  # noqa: E402

NODE_CLASS_MAPPINGS.update(_NODE_MAPPINGS)
NODE_DISPLAY_NAME_MAPPINGS.update(_NODE_NAME_MAPPINGS)
