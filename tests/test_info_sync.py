...
STALE_PHRASES: list[tuple[str, str]] = [
    ...
    ("视频参考消费仍未标定",
     "09-25 本机实测已标定视频：FL2VA 可迁移动作语义（红球下落测试，同 seed 带参考"
     "出现下落轨迹、无参考基线无），语义级跟随非精确复刻；精确动作/运镜主口径 edit 档"),
    ("对视频参考的消费未标定",
     "同上，09-25 已标定：FL2VA 视频参考不迁移动作（基线同样弹跳、衰减类似）"),
    ("可迁移动作语义",
     "09-25 二次判读推翻的误判口径：抽帧初判「带参考才有下落轨迹」被人工看片推翻——"
     "同 seed 基线同样弹跳且衰减类似，参考仅构图/细节级扰动；FL2VA 视频参考不构成动作迁移"),
    ("语义级跟随",
     "同上，误判口径退役：FL2VA 视频参考不迁移动作"),
    ("参考视频/音频槽已接入",
     "v1.24.0 起生成系（minimax-h3 / -lift）音视频参考槽已拆除（实测不迁移内容），"
     "槽位仅在 Ref2VA 系三支"),
    ("槽位在 H3 六支全部接入",
     "同上：v1.24.0 起 H3 视频档槽位 = Ref2VA 系三支，生成系传了 400"),
    ("生成系传了 400",
     "v1.25.0 起改为自动换档：minimax-h3 / -lift 带 reference_videos/audios 时路由到对应"
     " -edit 执行（仅 fasth3 无路由仍 400）"),
    ("已拆槽、传了 400",
     "同上，v1.25.0 路由口径：单一入口，带音视频参考自动换 edit 档"),
]
