...
>
> 当前版本：`1.22.0` ｜ 网关即 ComfyUI 自身（custom node），随 ComfyUI 启动自动加载。
>
...
| `reference_videos` | string[]? | 参考视频，最多 3：视频编辑 / 动作 / 运镜参考。槽位在 H3 六支全部接入。**09-25 实测**：FL2VA 权重（minimax-h3 / -lift）**可迁移动作语义**（红球下落测试：带参考输出出现下落落案轨迹、同 seed 无参考基线无此轨迹），为语义级跟随非精确复刻；精确动作 / 运镜迁移主口径走 Ref2VA 权重（`minimax-h3-edit` / `-lift-edit`）。无视频槽的模型（fasth3）传了 400 |
