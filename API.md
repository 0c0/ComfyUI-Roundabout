...
>
> 当前版本：`1.23.0` ｜ 网关即 ComfyUI 自身（custom node），随 ComfyUI 启动自动加载。
>
...
| `reference_videos` | string[]? | 参考视频，最多 3：视频编辑 / 动作 / 运镜参考。槽位在 H3 六支全部接入。**09-25 实测（红球弹跳 A/B + 人工判读）**：FL2VA 权重（minimax-h3 / -lift）**不迁移动作**——同 seed 无参考基线同样弹跳且衰减类似，参考仅带来球体取景大小、滚动方向等构图/细节级扰动；动作 / 运镜迁移主口径走 Ref2VA 权重（`minimax-h3-edit` / `-lift-edit`）。无视频槽的模型（fasth3）传了 400 |
