...
class VideoGenerationRequest(BaseModel):
    ...
    reference_videos: list[str] | None = Field(None, description="参考视频（最多 3 个）：视频编辑 / 动作 / 运镜参考；09-25 实测 FL2VA 权重不迁移动作（同 seed 基线同样弹跳且衰减类似，参考仅构图/细节级扰动），动作/运镜迁移主口径走 minimax-h3-edit；支持 base64/http(s)/本地路径")
